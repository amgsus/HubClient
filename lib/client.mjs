...
export class HubClient extends EventEmitter {
    ...
    constructor(opts) { super();
        ...
        this.#opts = {
            ...
            allowReceivedEmpties: this.#allowReceivedEmpties
    ...
    retrieve(
        ...
        if (callback === HubClient.noCallback) { callback = null; }
            ...
            this.#writeSerializedOrEnqueue({ // Potential fail if client is disconnected?
                ...
                callback: (errIO) => {
                    if (errIO) {
                        ...
                        this.awaitNotificationOrThrow(keyName, HubClient.noCallback, timeout).then((notification) => {
                            let { value } = notification;
                            if (!this.#allowReceivedEmpties && (value === "" || typeof value === "undefined")) {
                                cb(HubClientError.createCode(ERR_VALUE_NOT_EXISTS));
                            } else {
                                cb(null, value, notification);
                            }
                        }).catch(cb);
                    }
                }
